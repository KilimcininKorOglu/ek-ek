...
use ek_ek_dataplane::{header, unknown};
...
fn the_v1_form_is_one_line_of_text() {
    ...
    .expect("v1 produces a header");
    let line = String::from_utf8(bytes.clone()).expect("the v1 form is text");
...
fn the_address_is_the_one_it_was_given() {
    ...
    let proxy = at("198.51.100.5:25");
    let one = header(ProxyProtocol::V2, at("192.0.2.10:56324"), proxy).unwrap();
    let other = header(ProxyProtocol::V2, at("203.0.113.99:41000"), proxy).unwrap();
    ...
    let one = header(ProxyProtocol::V1, at("192.0.2.10:56324"), proxy).unwrap();
    let other = header(ProxyProtocol::V1, at("203.0.113.99:41000"), proxy).unwrap();
    assert_ne!(one, other);
...
fn a_pair_spanning_two_families_states_no_address() {
    ...
    .expect("v2 still produces a header");
...
fn an_unreadable_socket_still_gets_a_header() {
    ...
    assert_eq!(
        unknown(ProxyProtocol::V1).expect("v1 produces a header"),
        b"PROXY UNKNOWN\r\n"
    );

    let bytes = unknown(ProxyProtocol::V2).expect("v2 produces a header");
    assert_eq!(&bytes[..12], &SIGNATURE);
